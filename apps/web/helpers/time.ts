...
export function weekLabel(w: Week): string {
  ...
  if (sameYear && sameMonth)
    return `${format(w.start, 'MMM d')} , ${format(w.end, 'd')} , ${format(w.end, 'yyyy')}`;
  if (sameYear)
    return `${format(w.start, 'MMM d')} , ${format(w.end, 'MMM d')} , ${format(w.end, 'yyyy')}`;
...
const ensureDate = (value: Date | string) => (isDate(value) ? value : parseISO(value as string));
