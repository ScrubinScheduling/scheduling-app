...
export default function TimesheetTable({ timesheets }: { timesheets: Timesheet[] }) {
  ...
  const formatTimesheetWorkDuration = (timesheet: Timesheet) => {
    ...
    const breakDuration =
      timesheet.startBreakTime && timesheet.endBreakTime
        ? differenceInMinutes(new Date(timesheet.endBreakTime), new Date(timesheet.startBreakTime))
        : 0;

    return formatDurationHM(
      new Date(timesheet.clockInTime),
      new Date(timesheet.clockOutTime),
      breakDuration
    );
  };
            ...
            <TableRow key={timesheet.id}>
              ...
              <TableCell>
                {isEditing ? (
                  ...
                  format(new Date(timesheet.clockInTime), 'hh:mm a')
                ) : (
                  '_ _'
                )}
              </TableCell>
              ...
              <TableCell>
                {isEditing ? (
                  <Input
                    ...
                    className="max-w-[200px]"
                  />
                ) : timesheet.startBreakTime ? (
                  format(new Date(timesheet.startBreakTime), 'hh:mm a')
                ) : (
                  '_ _'
                )}
              </TableCell>
              ...
              <TableCell>
                {isEditing ? (
                  <Input
                    ...
                    className="max-w-[200px]"
                  />
                ) : timesheet.endBreakTime ? (
                  format(new Date(timesheet.endBreakTime), 'hh:mm a')
                ) : (
                  '_ _'
                )}
              </TableCell>
              ...
              <TableCell>
                {isEditing ? (
                  ...
                  format(new Date(timesheet.clockOutTime), 'hh:mm a')
                ) : (
                  '_ _'
                )}
              </TableCell>
              ...
              <TableCell className="text-right">
                {isEditing ? (
                  ...
                  <Button size="sm" variant="ghost" onClick={() => handleEdit(timesheet)}>
