...
function mapMeetingForResponse(meeting: any, currentUserMembershipId?: number) {
    ...
    for (const invite of meeting.invitations ?? []) {
        ...
        const name =
            [user?.firstName, user?.lastName].filter(Boolean).join(' ') || user?.email || 'Unnamed'
...
router.post('/', async (req, res) => {
    try {
        ...
        if (!location || !scheduledAt) {
            return res.status(400).json({ error: 'location and scheduledAt are required' })
        }
        ...
        if (Array.isArray(inviteMembershipIds)) {
            inviteIds = inviteMembershipIds.map((v) => Number(v)).filter((n) => !Number.isNaN(n))
        }
...
router.post('/:id/respond', async (req, res) => {
    try {
        ...
        if (!response || !validResponses.includes(response.toUpperCase())) {
            return res.status(400).json({
                error: 'Invalid response value. Must be "YES" or "NO".',
            })
        }

        // Check if this user has an invite for this meeting
        const invite = await prisma.meetingInvite.findFirst({
            where: { meetingId, membershipId: membership.id },
        })

        if (!invite) {
            return res.status(404).json({
                error: 'No invitation found for this meeting',
            })
        }
        ...
        const updated = await prisma.meetingInvite.update({
            where: { id: invite.id },
            data: {
                response: response.toUpperCase() as 'YES' | 'NO',
        ...
        console.error('POST /meetings/:id/respond error', err)
        return res.status(500).json({ error: 'Internal server error' })
    }
})

/**
 * POST /workspaces/:workspaceId/meetings/:id/respond/reset
 * Clear or change a user's response to a meeting invite.
 */
router.post('/:id/respond/reset', async (req, res) => {
    const { userId } = getAuth(req)
    const workspaceId = parseWorkspaceId(req)
    const meetingId = parseMeetingId(req)
    ...
    if (!invite) {
        return res.status(404).json({ error: 'No invitation found for this meeting' })
    }
    ...
    return res.status(200).json({
        message: 'Response reset to PENDING',
        invite: updated,
    })
})
